...
class SpotifyClient:
    ...
    async def add_tracks_to_playlist(self, playlist_id: str, track_uris: List[str], position: int = 0) -> str:
        ...
        response = await api.request_to_add_tracks(playlist_id, track_uris, position, self._access_token)
        logger.debug(
    ...
    async def _create_playlist(self, playlist_info: PlaylistInfo) -> str:
        ...
        response = await api.request_to_create_playlist(
            self._user_id,
            playlist_info['name'], playlist_info['description'], playlist_info['public'],
            self._access_token
        )
        return response['id']

    async def _search_for_track_uri(self, track_name: str) -> Optional[str]:
        response = await api.request_to_search_for_track(track_name, SINGLE_TRACK, self._access_token)
        ...
        logger.warning("Unknown fields in response from Spotify search API")
