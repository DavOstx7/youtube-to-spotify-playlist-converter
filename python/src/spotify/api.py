...
@http_request(expected_status_codes=[StatusCodes.OK], max_tries=1)
...
async def request_to_create_playlist(user_id: str, name: str, description: str, public: bool, access_token: str):
...
async def request_to_search_for_track(name: str, limit: int, access_token: str):
...
@http_request(expected_status_codes=[StatusCodes.CREATED])
async def request_to_add_tracks(playlist_id: str, track_uris: List[str], position: int, access_token: str):
...
def authorize_via_browser(client_id: str, redirect_uri: str):
    ...
    webbrowser.open(f"{SPOTIFY_AUTHORIZATION_URL}?{query_params}")
