...
class PlaylistsConverter:
    ...
    async def run(self) -> List[SpotifySnapshotIdsT]:
        ...
        async for titles_batch in self._youtube_client.walk_playlists_titles(YOUTUBE_PLAYLIST_IDS):
            add_titles_to_playlists_tasks.append(self._add_titles_to_playlists(titles_batch))
    ...
    async def _add_titles_to_playlists(self, track_names: List[str]) -> SpotifySnapshotIdsT:
